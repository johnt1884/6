    ...
    if ($totalProjectIssues -eq 0) {
        ...
        if ($projectIssues.MissingRegular.Count -gt 0) {
            ...
            $projectIssues.MissingRegular | ForEach-Object {
                ...
                $thumbPath = Join-Path $regularThumbsDir $thumbName
                $fixCommands += "ffmpeg -y -noautorotate -i ""$videoPath"" -ss 00:00:02.000 -update 1 -frames:v 1 -vf ""scale=${thumbWidth}:${thumbHeight}:force_original_aspect_ratio=decrease"" -map_metadata -1 ""$thumbPath"""
            }
        }
                ...
                if ($video) {
                    ...
                    if ($thumbPath.Contains('Edit Thumbnails')) {
                        try {
                            ...
                            $fixCommands += "ffmpeg -y -noautorotate -ss $timestamp_recalc -i ""$($video.FullName)"" -update 1 -vframes 1 -vf ""scale=${thumbWidth}:${thumbHeight}:force_original_aspect_ratio=decrease"" -map_metadata -1 ""$thumbPath"" >nul 2>&1"
                        } catch {
                     ...
                     $fixCommands += 'if exist "' + $thumbPath + '" del "' + $thumbPath + '"'
                }
            }
        }
        if ($projectIssues.ObsoleteRegular.Count -gt 0) {
            ...
            $projectIssues.ObsoleteRegular | ForEach-Object { $fixCommands += 'if exist "' + $_ + '" del "' + $_ + '"' }
        }
         if ($projectIssues.ObsoleteEdit.Count -gt 0) {
            ...
            $projectIssues.ObsoleteEdit | ForEach-Object { $fixCommands += 'if exist "' + $_ + '" del "' + $_ + '"' }
        }
    }
}
